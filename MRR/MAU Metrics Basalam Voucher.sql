...
, purchase_nbr1 as (
    select pi.user_id, pi.paid_at, pi.id, pi.rnk,
           sum(coalesce(basalam_product_discount,0)+
               coalesce(basalam_delivery_discount,0)+
               coalesce(public_delivery_discount,0)) as amount1
    ...
    group by pi.user_id, pi.paid_at, pi.id, pi.rnk
)
, purchase_nbr as (
    select id,paid_at,rnk,user_id, amount1+coalesce(credit,0) as amount
    from purchase_nbr1 a
    left join (
        select sp.invoice_id, sum( sp.charged_amount) as credit
        from laravel.spent_credits sp
        join laravel.credits c on sp.credit_id = c.id and c.deleted_at isnull
        where c.credit_type in (3570,3423,3346,3495,3343,4017)
        and sp.applied_at NOTNULL
        group by sp.invoice_id) d on id = invoice_id
)
, jmonth_userid as (
...
select nr.month_nbr, nr.jmonth,
       nr.new, nr.resurrected, nr.retained,
       nr.new+nr.resurrected+nr.retained as month_gmv,
