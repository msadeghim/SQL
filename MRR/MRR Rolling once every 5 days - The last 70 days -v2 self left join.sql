...
--MRR Rolling once every 5 days - The last 70 days
--v2 self left join
...
, jmonth_userid0 as (
    select day::date as day,
           ...
           min(rnk) as rnk
    from generate_series('2021-02-21'::date+ interval '5 day' * ((current_date-'2021-06-01'::date)/5*5),
...
, jmonth_userid as (
    select cur.*,
           per.user_id as backward_cheking,
           next.user_id as forward_cheking
    from jmonth_userid0 cur
    left join jmonth_userid0 per on per.day+30=cur.day and cur.user_id = per.user_id
    left join jmonth_userid0 next on next.day=cur.day+30 and cur.user_id = next.user_id
)
, lable as (
