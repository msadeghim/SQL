...
select *,
       ...
       round(100.0*(new_cnt+resurrected_cnt)/(lag(tot_month_customer)over(order by jmonth)-retainded_cnt),2) as quick_ratio,
       round(100.0*retainded_cnt/lag(tot_month_customer)over(order by jmonth)) as retention_ratio,
       tot_month_customer - lag(tot_month_customer) over(order by jmonth) as net_add,
       sum(new_cnt) over(order by jmonth) + 72309 as cumulative_customer,
       round(100.0*tot_month_customer /  (sum(new_cnt) over(order by jmonth) + 72309),2) as month_customer_to_cum
from fil)
select *
from f
where jmonth >= '1399-01'
order by jmonth
